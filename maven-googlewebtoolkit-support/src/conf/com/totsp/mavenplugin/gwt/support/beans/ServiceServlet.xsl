...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    -->
    <xsl:template match="/">
        package <xsl:value-of select="concat($destinationPackage, '.server')" />;
        import com.google.gwt.user.client.rpc.RemoteService;
        import com.google.gwt.user.server.rpc.RemoteServiceServlet;
        import com.totsp.gwt.beans.server.BeanMapping;
        import com.totsp.gwt.beans.server.MappingException;
    ...
    <xsl:template name="service">
        public class <xsl:value-of select="string(./shortName)" />Servlet extends RemoteServiceServlet 
        implements <xsl:value-of select="concat(concat($destinationPackage, '.client.'), string(./shortName))" />
        {
            private <xsl:value-of select="string(./implementation)" /> service = new <xsl:value-of select="string(./implementation)" />();
            private static final java.util.Properties mappings = new java.util.Properties();
            
            static {
                mappings.put( "<xsl:value-of select="string(./package)" />.*", "<xsl:value-of select="$destinationPackage" />.*" );
                <xsl:for-each select="mapping">
                    mappings.put( "<xsl:value-of select="string(./from)" />.*", "<xsl:value-of select="string(./to)" />.*" />);
                </xsl:for-each>
            }
    ...
    <xsl:template name="method">
        ...
        <xsl:for-each select="./argument">
            ...
            if( service instanceof com.totsp.gwt.beans.server.HttpService ){
                com.totsp.gwt.beans.server.HttpService httpService = (com.totsp.gwt.beans.server.HttpService) service;
                httpService.setThreadLocal( this.getThreadLocalRequest(), this.getThreadLocalResponse() );
            }
            try {
            <xsl:if test="count(./returnType) &gt; 0" >
                <xsl:for-each select="./returnType"><xsl:call-template name="type" /> returnValue = (<xsl:call-template name="type" />) </xsl:for-each>  
            </xsl:if>
            <xsl:if test="count(./returnType) &gt; 0 and
            ...
            <xsl:for-each select="./argument" >
                <xsl:if test="string(./shortName)!='int' and
                        ...
                        (<xsl:call-template name="rawType" />) BeanMapping.convert( mappings, 
                </xsl:if>
            ...
            } catch( <xsl:call-template name="rawType" /> e ){
                this.log( "Service exception" , e );
                <xsl:call-template name="type"/> exception =  new <xsl:call-template name="type"/>( e.getMessage() );
            </xsl:for-each>
            } catch( IntrospectionException e ){
                this.log( "Exception in conversion" , e );
    ...
    <xsl:template name="rawType">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:value-of select="string(./package)" />.<xsl:value-of select="./shortName" /><xsl:call-template name="array" />
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
