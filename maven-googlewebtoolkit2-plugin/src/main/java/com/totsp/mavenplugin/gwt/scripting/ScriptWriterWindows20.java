    ...
    public File writeI18nScript(AbstractGWTMojo mojo)
        ...
        if (!file.exists()) {
            ...
            try {
                ...
                if (mojo.getLog().isDebugEnabled()) {
                    mojo.getLog().debug("New file '" + file.getAbsolutePath()
                            + "' created.");
                }
            } catch (Exception exe) {
                mojo.getLog().error("Couldn't create file '" + file.getAbsolutePath()
    ...
    public void writeTestScripts(AbstractGWTMojo mojo)
        ...
        for (String currRoot : testCompileRoots) {
            ...
            for (File currFile : coll) {
                ...
                mojo.getLog().debug(("gwtTest test match found (after filter applied) - "
                        + testName));
