...
public class BeanGeneratorMojo extends AbstractGWTMojo {
   ...
   public void execute() throws MojoExecutionException, MojoFailureException {
      ...
      */      
      if (getProject().getProperties().getProperty(GOOGLE_WEBTOOLKIT_HOME) == null) {
         throw new MojoExecutionException("google.webtoolkit.home property not set");
      }
      ...
      try {
         ...
         for (int i = 0; i < rootClasses.length; i++) {
            ...
            BeanGeneratorBase.writeBean(this.getGeneratorDestinationPackage(), packageDirectory, this
                     .isGenerateGettersAndSetters(), this.isGeneratePropertyChangeSupport(), this
                     .isOverwriteGeneratedClasses(), root);

            if (getTranslatorDestinationPackage() != null) {
               if (translationGenerator == null) {
                  System.out.println("Generating Translator");
                  global = new GlobalGeneratorContext(isGenerateGettersAndSetters(), isGeneratePropertyChangeSupport(),
                           isOverwriteGeneratedClasses());
                  ...
                  translationGenerator = new TranslatorGenerator(getTranslatorDestinationPackage(), tranlatorPath,
                           getGeneratorDestinationPackage(), isTranslatorTwoWay());
                  translationGenerator.Init(global);
               }
               BeanGeneratorContext beanContext = new BeanGeneratorContext(global, root, translationGenerator,
                        translationGenerator.getImportMap());
