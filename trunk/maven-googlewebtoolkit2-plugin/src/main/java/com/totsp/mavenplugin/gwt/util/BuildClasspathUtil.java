...
public class BuildClasspathUtil {
    ...
    public static Collection<File> buildClasspathList(
        ...
        if (scope == DependencyScope.RUNTIME) {
            for (Iterator it = project.getRuntimeClasspathElements().iterator();
                    it.hasNext();) {
        ...
        // add system 
        for (Iterator it = project.getSystemClasspathElements().iterator();
                it.hasNext();) {
            items.add(new File(it.next().toString()));
        }
    ...
    public static Collection<File> injectGwtDepsFromRepo(
        ...
        mojo.getLog()
            .debug("injecting gwt-user and gwt-dev for script classpath from local repository (and expecting relative native libs)");

        Collection<File> items = new LinkedHashSet<File>();

        Artifact gwtUser = mojo.getArtifactFactory()
                               .createArtifactWithClassifier("com.google.gwt",
                "gwt-user", mojo.getGwtVersion(), "jar", null);
        Artifact gwtDev = mojo.getArtifactFactory()
                              .createArtifactWithClassifier("com.google.gwt",
                "gwt-dev", mojo.getGwtVersion(), "jar",
        ...
        try {
            mojo.getResolver()
                .resolve(gwtUser, remoteRepositories, mojo.getLocalRepository());
            mojo.getResolver()
                .resolve(gwtDev, remoteRepositories, mojo.getLocalRepository());
            ...
            throw new MojoExecutionException("artifact resolver problem - " +
                e.getMessage(), e);
        }
     ...
     * @return
     */
    private static List getSourceRoots(final MavenProject project,
        final DependencyScope scope) {
        if (DependencyScope.COMPILE.equals(scope)) {
    ...
    private static void addResourcesWithActiveProjects(
        final MavenProject project, final Set<File> items,
        final DependencyScope scope) {
        final List<Artifact> scopeArtifacts = BuildClasspathUtil.getScopeArtifacts(project,
                scope);

        BuildClasspathUtil.addResources(items,
        ...
        for (Artifact artifact : scopeArtifacts) {
            if (artifact instanceof ActiveProjectArtifact) {
                MavenProject refProject = (MavenProject) project.getProjectReferences()
                                                                .get(BuildClasspathUtil.getProjectReferenceId(
    ...
    private static void addSourcesWithActiveProjects(
        ...
        BuildClasspathUtil.addSources(items,
            BuildClasspathUtil.getSourceRoots(project, scope));
        ...
        for (Artifact artifact : scopeArtifacts) {
            if (artifact instanceof ActiveProjectArtifact) {
                ...
                if (refProject != null) {
                    BuildClasspathUtil.addSources(items,
                        BuildClasspathUtil.getSourceRoots(refProject, scope));
                }
            }
        }
    }
